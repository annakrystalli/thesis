...
load(file=paste(input.folder,"X_oc_data.RData", sep=""))
load(file=paste(input.folder,"y_oc_",spp,"_data.RData", sep=""))
...
trainNN<-function(x, y, x.val, y.val, size, decay, maxit,  
  ...
  nnet.mod$fitted.values<-cbind(1-nnet.mod$fitted.values, nnet.mod$fitted.values)
  ...
  hx.vp<-cbind(1-hx.vp, hx.vp)
  hx.v<-max.col(hx.vp)-1
...
adaboostOC<-function(X, y, x.val, y.val, size, decay=0.1, maxit=100, 
  ...
  adaboost.oc.nnets<-vector("list", T)
        ...
        for(t in 1:T){
                  ...
                  if(detail.metrics==TRUE){
                    metrics.c[t,]<-metricFy(y=y.neg, hx=sign(hxt%*%a))
                  ...
                  #save nnets
                  save(adaboost.oc.nnets,  file=paste(output.folder,"models/",file.ID, "_AB_oc_nnets.RData", sep=""))
                  
                  #save metrics
                  if(detail.metrics==TRUE){
                  save(Hx, Hx.v, Hx.p, Hx.vp, ABmetrics, metrics.cvp, metrics.cv, metrics.c, metrics.cp,
                       file=paste(output.folder,file.ID, "_AB_oc_outputs.RData", sep=""))}else{
                         save(Hx, Hx.v, Hx.p, Hx.vp, ABmetrics,  
                              file=paste(output.folder,file.ID, "_AB_oc_outputs.RData", sep=""))      
                       }
