...
load(file="/Users/annakrystalli/Documents/TRAINING DATA/X_oc_data.RData")
load(file="/Users/annakrystalli/Documents/TRAINING DATA/y_oc_chel_data.RData")
...
metricFy<-function(y, hx){
  if(length(unique(hx))==3){
    y<-y[hx!=0]
    hx<-hx[hx!=0]
  }
...
#_________________________________________________________________________________________________________________________
trainNN<-function(x, y, x.val, y.val, size, decay, maxit,  
                  varnames, weights){
  ...
  hx.v<-max.col(predict(nnet.mod, newdata=x, type="raw"))-1
...
#_________________________________________________________________________________________________________________________
adaboostOC<-function(X, y, x.val, y.val, size, decay=0.1, maxit=100, 
                     varnames,file.ID=NULL, T, K, detail.metrics=TRUE){
  ...
  for(t in 1:T){
    ...
    if(t==T){
      ...
      ABmetrics<-rbind(metricFy(y=y.neg.val, hx=Hx.v), metricFy(y=y.val, hx=Hx.vp))
      rownames(ABmetrics)<-c("hxv", "hxvp")
      
      file.ID<-paste(size, signif(decay, digits=2), sep="-")
      
      #save nnets
      save(adaboost.oc.nnets,  file=paste("~/Documents/TRAINING DATA/Models/",file.ID, "AB_oc_nnets.RData", sep=""))
      
      #save metrics
      if(detail.metrics==TRUE){
        save(Hx, Hx.v, Hx.p, Hx.vp, ABmetrics, metrics.cvp, metrics.cv, metrics.c, metrics.cp
             file=paste("~/Documents/TRAINING DATA/Outputs/",file.ID, "AB_oc_outputs.RData", sep=""))}else{
               save(Hx, Hx.v, Hx.p, Hx.vp, ABmetrics,  
                    file=paste("~/Documents/TRAINING DATA/Outputs/",file.ID, "AB_oc_outputs.RData", sep=""))      
             }
...
ensembleOC<-function(X, y, x.val, y.val, size, decay=0.1, maxit=100, 
                     varnames,file.ID=NULL, T, detail.metrics=TRUE){
  ...
  for(t in 1:T){
    ...
    if(t==T){
      ...
      file.ID<-paste(size, signif(decay, digits=2), sep="_")
      
      save(ensemble.oc.nnets,  file=paste("~/Documents/TRAINING DATA/Models/",file.ID, "ES_oc_nnets.RData", sep=""))
      ...
      if(detail.metrics==TRUE){
        save(Hx, Hx.v, Hx.p, Hx.vp, Hxa, Hxa.v, Hxa.p, Hxa.vp, 
             ...
             metrics.cva,
             metrics.cvpa, 
             file=paste("~/Documents/TRAINING DATA/Outputs/",file.ID, "ES_oc_outputs.RData", sep=""))}else{
               save(Hx, Hx.v, Hx.p, Hx.vp, Hxa, Hxa.v, Hxa.p, Hxa.vp, ESmetrics, metrics.t,  
                    file=paste("~/Documents/TRAINING DATA/Outputs/",file.ID, "ES_oc_outputs.RData", sep=""))      
             }
...
n<-dim(X.o.tr.m)[1]
ensembleOC(X=X.o.tr.m, y=y.o.tr.m, x.val=X.o.v.m, y.val=y.o.v.m, size=25, decay=0.1, maxit=20000, 
...
adacost2OC<-function(X, y, x.val, y.val, size, decay=0.1, maxit=100, 
                     varnames,file.ID=NULL, T, T.test, C.vec, detail.metrics=TRUE){
  ...
  for(t in 1:T){
    ...
    #Calculate nn[t] weight
    a[t]<-(1/2)*log((II*D[,t]*Iic)/(I*D[,t]*Iic))
  ...
  for(i in 1:length(C.vec)){
    ...
    if(i==length(C.vec)){
      ...
      save(C2.test.metrics, 
           file=paste("~/Documents/TRAINING DATA/Outputs/C2_cost_setup_test_outputs.RData", sep=""))      
      #set C  
...
n<-dim(X.o.tr.m)[1]
C<-adacost2OC.test(X=X.o.tr.m, y=y.o.tr.m, x.val=X.o.v.m, y.val=y.o.v.m, size=15,
                   decay=0.1/n, maxit=20000, 
                   varnames=v1, T.test=5, C.vec=C.vec)
