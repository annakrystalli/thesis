...
new.x<-levels(x.dat[, "spl.id"])
new.x<-cbind(new.x, c("day", "night")[as.numeric(unlist(lapply(split(x.dat[,"time"], 
                                               x.dat[, "spl.id"]), 
                                             FUN=unique)))])
new.x<-as.data.frame(new.x)

for(i in 1:length(varnames.t)){
new.x<-cbind(new.x, 
             unlist(lapply(split(x.dat[,varnames.t[i]], 
                                 x.dat[, "spl.id"]), FUN=mean)))}

names(new.x)<-c("spl.id", "time", varnames.t)
...
for(i in 1:5){
y<-cbind(y, unlist(lapply(split(y.m[,i],x.dat[, "spl.id"]), FUN=mean)))}
y<-as.data.frame(y)
names(y)<-names.spp
y.m<-y

x.dat<-new.x
all.samples<-x.dat[, "spl.id"]
      ...
      lapply(split(y.m[, spp.id], all.samples), mean)) 
                                                                             ...
                                                                             test.samples<-sort(unlist(lapply(split(samples,samp.means),
                                                                                                              FUN=function(x, test.size){
                                                                                                                sample(x, size=round(test.size*length(x)))},
                                                                                                              test.size=test.size)))
...
GrowRFs<-function(x.dat, y.m, tune, spp.id, all.samples,
                  varnames, test.size=0.2, model){
...
compileMeasures<-function(res=res){
  ...
  res.oc<-matrix(0,14,2, dimnames=list(measures.oc, 
                                       c("RF_M.tr",  "RF_M.t")))
  ind<-c(0, 1)
  for(i in 1){  
  ...
  res.ac<-matrix(0,13,2, dimnames=list(measures.ac, 
                                       c("RF_M.tr",  "RF_M.t")))
  ind<-c(0, 1)
  for(i in 1){  
