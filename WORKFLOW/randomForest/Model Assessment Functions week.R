...
GrowRFs<-function(x.dat, y.m, tune, tune.c, tune.b, spp.id, all.samples,
                  varnames, test.size=0.2){
  
  set.seed(1)
  #....DEPENDENT TEST DATA.....   
  
  rf.dat<-testTrainSplit(all.samples, y.m, test.size=test.size,
  ...
  return(list(rfOC, rfAC, rfOC.c, rfAC.c,rfOC.b, rfAC.b))}
...
measurePerformance<-function(y, pr, probs, type, rf){
  ...
  var.order<-rev(order(importance(rf)))
