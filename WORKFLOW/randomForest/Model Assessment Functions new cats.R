...
GrowRFs<-function(x.dat, y.m, tune.i,  tune.b, spp.id, all.samples,
                  varnames, test.size=0.2, model, ntree, ntree.b){
...
compileMeasures<-function(res=res){
  ...
  res.oc<-matrix(0,14,4, dimnames=list(measures.oc, 
                                       c("RF_I.tr",  "RF_I.t", "RF_B.tr", 
                                         "RF_B.t")))
  ind<-c(0, 1)
  for(i in c(1,2)){  
  ...
  res.ac<-matrix(0,13,4, dimnames=list(measures.ac, 
                                       c("RF_I.tr",  "RF_I.t", "RF_B.tr", 
                                         "RF_B.t")))
  ind<-c(0, 1)
  for(i in 1:2){  
...
compileVarImp<-function(an.ID){
  ...
  for(spp.id in 1:5){
    ...
    OC<-matrix(0, 16, 2, dimnames=list(variables, c("RF_I","RF_B")))
    ...
    AC<-matrix(0, 16, 2, dimnames=list(variables, c("RF_I","RF_B")))
