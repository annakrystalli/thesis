...
plotMaps<-function(x, cats=5, fix.cols=T, BrCol=BrCol){
  if(fix.cols){
    cols <- brewer.pal(cats, name=BrCol) 
    cols <- c("#F0FFFF", "#EEE0E5", cols[-1])
...
plotValMaps <- function(an.ID, y.m, x.dat, id, w.spp, spp, pt1=12, pt2=14, 
                        pred.map=pr.map.night, map.med, map.int.ac, map.int.con, map.win,
                        BrCol=BrCol){
  
  dir.create(paste("~/Documents/TRAINING DATA/Models/randomForest/",
                   an.ID,"time corrected/", sep=""),
             showWarnings = F)
  dir.create(paste("~/Documents/TRAINING DATA/Models/randomForest/",
                   an.ID,"time corrected/validation plots/", sep=""),
             showWarnings = F)
  dir.create(paste("~/Documents/TRAINING DATA/Models/randomForest/",
                   an.ID,"time corrected/validation plots/",spp, "/", sep=""),
             showWarnings = F)
  
  y<-y.m[x.dat$m.id==id,w.spp]
  x<-x.dat[x.dat$m.id==id, c("lon", "lat", "time") ]
  
  png(filename = paste("~/Documents/TRAINING DATA/Models/randomForest/",
                       an.ID,"time corrected/validation plots/", spp,"/",thresh,id, " val hist.png", 
                       sep=""), 
  ...
  png(filename = paste("~/Documents/TRAINING DATA/Models/randomForest/",
                       an.ID,"time corrected/validation plots/", spp,"/",thresh,id, " val maps.png", 
                       sep=""), 
  ...
  t.pch<-c(0.1,1.3)[as.factor(x$time)]
  
  map("world", xlim=c(-4,11), 
      ylim=c(51,61), fill=TRUE,
      resolution=0)
  points(x, col=y.cols, bg=y.cols, pch=21, cex=t.pch)
  axis(2, seq(51,61, l=diff(range(c(51,61)))+1), font=2)
  axis(1, seq(-4,11, l=diff(range(c(-4,11)))+1), font=2)
  
  
  plotMaps(pred.map, cats=5, fix.cols=T, BrCol=BrCol)
  map("world", xlim=c(-4,11), 
      ylim=c(51,61), fill=TRUE, col="white",
      resolution=0, add=T)
  
  plotMaps(map.med, cats=5, fix.cols=T, BrCol=BrCol)
  map("world", xlim=c(-4,11), 
      ylim=c(51,61), fill=TRUE, col="white",
      resolution=0, add=T)
  
  plotMaps(map.win, cats=5, fix.cols=T, BrCol=BrCol)
  map("world", xlim=c(-4,11), 
      ylim=c(51,61), fill=TRUE, col="white",
      resolution=0, add=T)
  
  plotMaps(map.int.con, cats=5, fix.cols=F, BrCol=BrCol)
  map("world", xlim=c(-4,11), 
      ylim=c(51,61), fill=TRUE, col="white",
      resolution=0, add=T)
  
  plotMaps(map.int.ac, cats=5, fix.cols=T, BrCol=BrCol)
  map("world", xlim=c(-4,11), 
      ylim=c(51,61), fill=TRUE, col="white",
      resolution=0, add=T)
...
winCPRValidateTC<- function(an.ID, an.IDCV, cv.no=5, names.spp, data=" mod",
                          model="rf_i", plot.id=F, thresh, inits, pr.time, BrCol="YlGnBu"){
  ...
  dir.create(path=paste("~/Documents/PREDICTED/",an.ID,"AORS/data/time corrected", 
                        sep=""), showWarnings = F)
  
  dir.create(path=paste("~/Documents/PREDICTED/",an.ID,
                        "AORS/data/time corrected/aggregated maps", 
                        sep=""), showWarnings = F)
  
  dir.create(path=paste("~/Documents/TRAINING DATA/Models/randomForest/",
                        an.ID,"time corrected", sep=""), showWarnings = F)
  ...
  for(spp in names.spp){
    ...
    dir.create(path=paste("~/Documents/PREDICTED/",an.ID,
                          "AORS/data/time corrected/aggregated maps/",spp, 
                          sep=""), showWarnings = F)
    ...
    val.dat<-x.dat[x.ind,c("lon","lat","tow","sample.no.","year","month", "time")]  
    ...
    for(id in unique(val.table$m.index)){
      ...
      assign(paste("pr.map", time, sep="."),
             raster(pred.map, -4,11,51,61, crs="+proj=longlat +datum=WGS84"))}
      ...
      # fitted values
      rfy.y.day<-pr.map.day[cbind(val.table[val.table$m.index==id & val.table$time=="day" , "r.ind"], 
                          val.table[val.table$m.index==id & val.table$time=="day", "c.ind"])]
      
      if(!is.null(rfy.y.day)){
        names(rfy.y.day)<-rownames(val.table[val.table$m.index==id & val.table$time=="day", ])}
      rfy.y.night<-pr.map.night[cbind(val.table[val.table$m.index==id & val.table$time=="night" , "r.ind"], 
                              val.table[val.table$m.index==id & val.table$time=="night", "c.ind"])]   
      if(!is.null(rfy.y.night)){
        names(rfy.y.night)<-rownames(val.table[val.table$m.index==id & val.table$time=="night",
                                               ])}
         
      rfy.tdat<-c(rfy.y.day, rfy.y.night)
      op<-initSearchOpt(rfy.tdat, inits, breaks, spp)
      AB.rfyt<-c(AB.rfyt, op$mean)
      nAC.rfyt<-c(nAC.rfyt, op$nAC)
      oc.rfyt<-c(oc.rfyt,sum(rfy.tdat>0)/length(rfy.tdat))
      
      rfy.y<-pr.map.night[cbind(val.table[val.table$m.index==id, "r.ind"], 
      ...
      map.int.con <- aggregate(pr.map.night, fact=c(71 , 50), 
      ...
      # using median      
      map.med <- aggregate(pr.map.night, fact=c(71 , 50), 
                           fun=median, 
                           expand=F)
      ...
      if(id %in% plot.id){plotValMaps(an.ID, y.m, x.dat, id, 
                                      ...
                                      pred.map=pr.map.night, map.med, 
                                      map.int.ac, map.int.con, map.win, BrCol=BrCol)}
      
      val.table[val.table$m.index==id, "rf.y"]<-pr.map.night[cbind(val.table[val.table$m.index==id, "r.ind"], 
                                                             val.table[val.table$m.index==id, "c.ind"])]
      
      val.table[names(rfy.tdat),"rf.yt"] <- rfy.tdat
    ...
    AOR.data<-cbind(oc.y, oc.rf, oc.win, AB.yav, AB.yint, nAC.yint,
                    ...
                    AB.rfyt, oc.rfyt, nAC.rfyt,
                    N.y, N.rf, N.win, yr, mm)
    
    
    save(AOR.data,file=paste("~/Documents/PREDICTED/", an.ID,"AORS/data/time corrected/",
                             thresh, spp," monthly AOR.Rdata",sep=""))}
  
  save(val.DATA, file=paste("~/Documents/TRAINING DATA/Models/randomForest/",
                            an.ID,"time corrected/", thresh,"validation data.RData", sep=""))
...
summariseValTC<-function(thresh, names.spp, an.ID){
  ...
  save(val.SUMMARY, file=paste("~/Documents/TRAINING DATA/Models/randomForest/",
                               an.ID, "/time corrected/", thresh, "validation summary.RData", sep=""))
}
