...
AORextract<-function(spp, a, b, y.plots=F, an.ID=NULL, 
                     years=NULL, season=F, no.plot=T, hists=T){
...
plotYearly<-function(pt=12, y.plots=F, spp=spp, 
                     an.ID=NULL, years=NULL, no.plot=F, PR=T){
...
interannualAOR<-function(plot=F, spp=spp, 
  ...
  x=unlist(lapply(split(AB, yr), mean))
  y=unlist(lapply(split(oc, yr), mean))
  n=unlist(lapply(split(N, yr), mean))
  y<-as.integer(y*n)/n
  std <- function(x) sd(x)/sqrt(length(x))
  
  x.sd=unlist(lapply(split(AB, yr), std))
  y.sd=unlist(lapply(split(oc, yr), std))
  n.sd=unlist(lapply(split(N, yr), std))
  ...
  glm.dat<-c(glm.all$coefficients, glm.all$deviance)
    ...
    if(no.plot==T){}else{
      ...
      main.lab<-paste(sp.names[spp.id], "inter-annual AOR") 
      cols.l<-FUN.c(n=length(years))
      cols<-cols.l[as.factor(yr)]
  ...
  if(PR==T){save(glm.dat, file=paste("~/Documents/PREDICTED/", an.ID,"AORS/data/",spp, log,
                                     " AOR PROC glm tab.RData", sep=""))}else{
                                       save(glm.tab, file=paste("~/Documents/PREDICTED/", an.ID,"AORS/data/",spp, log,
                                                                " AOR glm tab.RData", sep=""))}
}
